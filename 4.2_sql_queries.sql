...
SELECT bc.type AS type_of_card, 
       bc.issue_date AS issue_date 
FROM bank.card AS bc;
...
SELECT * 
FROM bank.loan
WHERE status = 'A'; -- Suponiendo que 'A' es el estado que indica que no fue pagado
...
SELECT district_name, 
       (num_inhabitants * urban_population_percentage / 100) AS urban_population
FROM bank.district;
...
SELECT district_name, 
       (num_inhabitants * urban_population_percentage / 100) AS urban_population
FROM bank.district
WHERE (100 - urban_population_percentage) <= 50;
...
SELECT * 
FROM bank.card
WHERE type = 'junior' AND issue_date > 980000;
...
SELECT * 
FROM bank.trans 
WHERE type = 'withdrawal' AND operation != 'cash'
ORDER BY date
LIMIT 10;
...
SELECT loan_id, account_id, amount - payments AS debt_value
FROM bank.loan
WHERE status = 'A' AND (amount - payments) > 1000
